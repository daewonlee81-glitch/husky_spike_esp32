...
# Source: https://github.com/antonvh/PUPRemote/blob/main/src/lpf2.py  (v1.5)
...
class LPF2(object):
    def __init__(
        ...
        self.payloads = {}
        # --- 콤보 모드(SPIKE3 색 센서 워드 블럭이 사용) 지원 ---
        self.combo = False        # 콤보 모드 활성 여부
        self.combo_raw = None     # 허브가 보낸 콤보 설정 패킷(튜플 파싱용)
        self.mode_values = {}     # {mode: [값들]}  콤보 응답 구성용
        self.last_nack = 0
        self.debug = debug
    ...
    def send_combo_payload(self):
        """허브가 콤보 설정(0x5C)에서 요청한 모드/값 순서 그대로 결합 페이로드 전송.
        튜플(예: 00 10 50 51 52 53 = 색상,반사광,R,G,B,4th)을 파싱해 각 값을
        모드의 데이터 타입(8/16비트)에 맞춰 채운다."""
        if not self.connected or not self.combo_raw or len(self.combo_raw) < 3:
            return
        count = self.combo_raw[0] & 0x1F
        tuples = self.combo_raw[2:2 + count]
        data = bytearray()
        for tup in tuples:
            mode = tup >> 4
            vidx = tup & 0x0F
            vals = self.mode_values.get(mode, ())
            v = vals[vidx] if vidx < len(vals) else 0
            dt = self.modes[mode][1][1] if mode < len(self.modes) else 0
            if dt == 0:                       # DATA8
                data.append(v & 0xFF)
            else:                             # DATA16
                data.append(v & 0xFF)
        ...
        while s > 1:
            s >>= 1
            bits += 1
        buf = bytearray([MSG_DATA | (bits << CMD_LLL_SHIFT) | 0]) + data
        ck = 0xFF
        for x in buf:
            ck ^= x
        buf.append(ck)
    ...
    def heartbeat(self):
        ...
        if b > 0:  # There is data, let's see what it is.
            if b == BYTE_NACK:
                ...
                # 콤보 모드면 결합 페이로드를, 아니면 현재 모드 페이로드를 보냄
                if self.combo:
                    self.send_combo_payload()
                else:
                    self.send_payload()

            elif b == 0x5C:
                # 콤보 모드 설정 패킷 (예: 5C 25 00 10 00 50 51 52 00 C5)
                ...
                self.combo = True
                self.combo_raw = rest
                # 받은 패킷을 그대로 ack 로 되돌려줌 (= 동일 패킷)
                self.write(bytearray([0x5C]) + rest)

            elif b == 0x4C:
                # 콤보 모드 해제 패킷 (4C 20 00 93)
                self.last_nack = utime.ticks_ms()
                for _ in range(3):
                    if self.readchar() < 0:
                        break
                self.combo = False
                self.write(bytearray([0x44, 0x20, 0x9B]))  # ack
